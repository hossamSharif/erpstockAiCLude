...
let ItemStockPage = class ItemStockPage {
    ...
    getExportColumns() {
        ...
        if (this.colSetting.part_no)
            columns.push({ key: 'part_no', title: 'رقم القطعة', width: 15, type: 'text' });
        if (this.colSetting.item_unit)
            columns.push({ key: 'item_unit', title: 'الوحدة', width: 10, type: 'text' });
        ...
        if (this.colSetting.pay_price)
            columns.push({ key: 'pay_price', title: 'سعر البيع', width: 12, type: 'currency' });
        if (this.colSetting.min_qty)
            columns.push({ key: 'min_qty', title: 'الحد الأدنى', width: 10, type: 'number' });
        if (this.colSetting.instock)
            columns.push({ key: 'quantity', title: 'الكمية المتاحة', width: 12, type: 'number' });
        if (this.colSetting.total)
            columns.push({ key: 'stockValue', title: 'قيمة المخزون', width: 15, type: 'currency' });
        return columns;
    }
};
