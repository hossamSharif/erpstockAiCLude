...
export class PurchaseReturnPage implements OnInit, OnDestroy {
  ...
  async loadAvailablePurchaseInvoices() {
    await this.showLoading('جاري تحميل فواتير الشراء المتاحة...', 'saving');
  ...
  async selectOriginalInvoice(invoice: any) {
    await this.showLoading('جاري فحص فاتورة الشراء المختارة...', 'saving');
    ...
    try {
      ...
      if (existingReturns.hasExistingReturns) {
        ...
        const shouldContinue = await this.presentConfirmAlert(
          ...
          `هذه الفاتورة لديها ${existingReturns.returnCount} مرتجعة(ات) موجودة بالفعل.\nهل تريد المتابعة لإنشاء مرتجعة جديدة؟`,
          'نعم، متابعة',
          'إلغاء'
        );
  ...
  async loadOriginalInvoiceItems(pay_ref: string) {
    await this.showLoading('جاري تحميل أصناف فاتورة الشراء...', 'saving');
  ...
  private async checkForExistingReturns(original_pay_ref: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.api.checkExistingPurchaseReturns(original_pay_ref, this.store_info.id, this.year.id).subscribe(
        (response: any) => {
          resolve(response);
        },
        (error) => {
  ...
  private async showExistingReturnsWarning(existingReturns: any) {
    if (existingReturns.hasFullReturn) {
      ...
      this.presentToast(
        `تحذير: توجد ${existingReturns.returnCount} مرتجعة(ات) لهذه الفاتورة.`, 
        'warning'
      );
    }
  }
