...
export class PurchaseReturnPage implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.returnInvo = {
      ...
      supplier_id: null, return_method: "", discount: 0, changee: 0, sub_name: "",
  ...
  loadAvailablePurchaseInvoices() {
    this.isSaving = true;
    this.currentLoadingMessage = 'جاري تحميل فواتير الشراء المتاحة...';
    this.cdr.detectChanges();
  ...
  async selectOriginalInvoice(invoice: any) {
    ...
    this.returnInvo.supplier_id = invoice.supplier_id || invoice.cust_id;
    this.selectedAccount.id = invoice.supplier_id || invoice.cust_id;
    this.selectedAccount.sub_name = invoice.sub_name || '';

    // Load original invoice items (critical path)
    this.loadOriginalInvoiceItems(invoice.pay_ref);

    // Check for existing returns in background (non-blocking)
    ...
    this.api.getPerchInvoDetail(this.store_info.id, pay_ref, this.year.id).subscribe(data => {
      ...
      try {
        ...
        if (this.originalItems.length > 0) {
          ...
          this.presentToast('لا توجد أصناف في فاتورة الشراء المحددة', 'warning');
        }
  ...
  getAppInfo() {
    Promise.all([
      ...
      if (storeInfo) {
        ...
        // Auto-select invoice if navigated from purchase-record, otherwise show invoice selector
  ...
  handleNavigationParams(params: any) {
    if (params['original_pay_ref']) {
      const mockInvoice = {
        ...
        supplier_id: params['supplier_id'] || params['cust_id'],
        cust_id: params['cust_id'],
        sub_name: params['supplier_name'] || params['cust_name'],
  ...
  validate(): boolean {
    ...
    // 2. Items validation - check checklist
    ...
    if (!this.returnInvo.supplier_id) {
      this.presentToast('لم يتم تحديد المورد من فاتورة الشراء الأصلية', 'danger')
      return false
    }
    ...
    if (this.returnInvo.changee < 0) {
      this.presentToast('الرجاء مراجعة المبلغ المدفوع والخصم', 'danger')
      return false
    }
  ...
  async saveReturn() {
    await this.showLoading('جاري حفظ فاتورة مرتجعات الشراء...', 'saving');
    ...
    try {
      ...
      this.api.createPurchaseReturnWithItems(returnWithItems).subscribe(
        async (response: any) => {
  ...
  async presentAlertConfirm() {
    let msg: string = 'هل تريد طباعة فاتورة مرتجعات الشراء ؟ '
  ...
  validateReturnQuantities(): {valid: boolean, message: string} {
    for (const returnItem of this.itemList) {
      ...
      if (!originalItem) {
        return {
          valid: false,
          message: `الصنف "${returnItem.item_name}" غير موجود في فاتورة الشراء الأصلية`
        };
      }
      ...
      if (+returnItem.return_price > (+originalItem.perch_price * 1.1)) {
        return {
          valid: false,
          message: `سعر الإرجاع للصنف "${returnItem.item_name}" أكبر من سعر الشراء الأصلي بشكل غير مقبول`
        };
      }
    }
  ...
  validateBusinessLogic(): {valid: boolean, message: string} {
    ...
    if (returnTotal > originalTotal) {
      return {
        valid: false,
        message: `إجمالي المرتجعة (${returnTotal}) لا يمكن أن يتجاوز إجمالي فاتورة الشراء الأصلية (${originalTotal})`
      };
    }
    ...
    const invoiceSupplierId = this.selectedOriginalInvoice.supplier_id || this.selectedOriginalInvoice.cust_id;
    if (invoiceSupplierId && invoiceSupplierId !== this.returnInvo.supplier_id) {
      return {
        valid: false,
        message: 'يجب أن يكون المورد المسترجع هو نفس مورد فاتورة الشراء الأصلية'
      };
    }
  ...
  // Search methods (kept for compatibility)
  ...
  buildChecklistFromOriginalItems() {
    this.checklistItems = this.originalItems.map(item => ({
      ...
      originalPrice: +(item.perch_price || item.pay_price),
      returnQty: +item.quantity,
      returnPrice: +(item.perch_price || item.pay_price),
      selected: true,
      rowTotal: +(+item.quantity * +(item.perch_price || item.pay_price)).toFixed(2),
      tax: item.tax || 0,
  ...
  onReturnPriceChange(index: number) {
    ...
    if (item.returnPrice > item.originalPrice * 1.1) {
      item.returnPrice = item.originalPrice;
      this.presentToast('سعر الإرجاع لا يمكن أن يتجاوز سعر الشراء الأصلي بأكثر من 10%', 'warning');
    }
